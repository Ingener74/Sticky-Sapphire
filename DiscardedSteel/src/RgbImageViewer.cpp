...
#include "DiscardedSteel/RgbImageViewer.h"
...
void RgbImageViewer::drawRgbImage( RgbImage rgbImage )
{
    ...
    if (res != JNI_OK)
        throw runtime_error("can't attach current thread");
    ...
    if (!string_Config_Name)
        throw runtime_error("can't create config string");

    jclass class_Bitmap_Config = jniEnv->FindClass("android/graphics/Bitmap$Config");
    if (!class_Bitmap)
        throw runtime_error("can't find bitmap config class");
    ...
    if (!object_Bitmap_Config)
        throw runtime_error("can't create bitmap config object");
    ...
    if (res != ANDROID_BITMAP_RESULT_SUCCESS)
        throw runtime_error("can't lock pixels");
    ...
    if (data)
    {
        ...
        if (res != ANDROID_BITMAP_RESULT_SUCCESS)
            throw runtime_error("can't unlock pixels");
    }
    ...
    if ((res = jvm->DetachCurrentThread()) != JNI_OK)
        throw runtime_error("can't detach current thread");
}
